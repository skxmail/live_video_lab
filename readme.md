...
2. **Configura las variables de entorno en `docker-compose.yaml`**
   ...
   - Cambia `MANIFEST_URL` por la URL del manifest que el monitor debe analizar.
   - Cambia las variables de entorno del packager para configurar los parametros de shaka packager.
...
# Monitor - Herramientas de Análisis de Streams

Contiene herramientas especializadas para analizar la calidad, latencia y adaptación de streams DASH en tiempo real.
...
└── analysis_suite.csv
```
Los .json tiene la estrauctura de datos json, los .csv es el mismo reporte del .json pero en formato tabular, los .txt son resumentes y el .png es un grafico de bitrate.
...
Los datos generados por las herramientas se integran automáticamente con el dashboard web via la carpeta mapeada de logs:
